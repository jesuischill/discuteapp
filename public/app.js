...
async function showAdmin() {
  ...
  document.getElementById("content").innerHTML = `
    ...
    <div class="card">
      ...
      <input id="adminSearchUser" placeholder="Pseudo..." maxlength="30">
      <button id="adminSearchButton">🔎 Rechercher</button>
      <div id="adminUserResult"></div>
    </div>

    <div class="card">
      <h2>📢 Annonce publique</h2>
      <p>Cette annonce apparaîtra dans le chat avec un encadrement doré.</p>
      <textarea
      ...
      ></textarea>
      <button id="adminAnnouncementButton">✨ Envoyer l'annonce</button>
    </div>

    <div class="card">
      <h2>💎 Gestion des gemmes</h2>
      <input id="gemsUsername" placeholder="Pseudo">
      <input id="gemsAmount" type="number" min="1" placeholder="Nombre de gemmes">
      <button id="adminAddGems">➕ Donner</button>
      <button id="adminRemoveGems" class="danger">➖ Retirer</button>
    </div>

    <div class="card">
      <h2>🗑️ Messages publics</h2>
      <button id="adminClearMessages" class="danger">
        🗑️ Supprimer tous les messages publics
      </button>
    </div>
  ...
  document.getElementById("adminSearchButton").onclick = searchAdminUser;

  document.getElementById("adminSearchUser").addEventListener("keydown", e => {
    if (e.key === "Enter") searchAdminUser();
  });

  document.getElementById("adminAnnouncementButton").onclick =
    sendAdminAnnouncement;

  document.getElementById("adminAddGems").onclick =
    () => manageGems("add");

  document.getElementById("adminRemoveGems").onclick =
    () => manageGems("remove");

  document.getElementById("adminClearMessages").onclick =
    clearPublicMessages;
  ...
  if (me.role === "owner" || me.username === "chilladmin") {
    ...
    const discountTitle = document.createElement("h2");
    ...
    document.getElementById("adminUsers").appendChild(discountBox);
    ...
    try {
      ...
      items.forEach(item => {
        ...
        option.textContent =
          `${item.name} — 💎 ${Number(item.price || 0).toLocaleString("fr-FR")}` +
          (item.discount > 0
            ? ` — 🔥 -${item.discount} %`
            : " — Prix normal");
...
async function loadAdminUsers() {
  try {
    ...
    users.forEach(user => {
      ...
      info.textContent =
        `Créé le : ${user.created_at || "inconnu"}`;
      card.appendChild(info);

      if (user.role !== "owner") {
        const ban = document.createElement("button");
        ban.textContent = user.banned ? "✅ Débannir" : "🚫 Bannir";
        ban.className = user.banned ? "success" : "danger";
        ban.onclick = () =>
      ...
      if (user.role !== "owner" && user.username !== "chilladmin") {
        const del = document.createElement("button");
        del.textContent = "🗑️ Supprimer définitivement";
        del.className = "danger";

        del.onclick = async () => {
          if (!confirm(
            `Supprimer définitivement ${user.username} ?\n\nToutes ses données liées seront supprimées.`
          )) return;

          try {
            await api(`/api/admin/users/${user.id}/delete-permanently`, {
              method: "POST"
            });
...
async function searchAdminUser() {
  ...
  try {
    const data = await api(
      `/api/admin/user-details?username=${encodeURIComponent(username)}`
    );
    ...
    info.textContent =
      `ID : ${user.id}
       ...
       | Gemmes : ${Number(user.gems || 0).toLocaleString("fr-FR")}
       | Créé le : ${user.created_at}
       | Mot de passe : 🔒 Protégé`;
    result.appendChild(info);

    const accessoriesTitle = document.createElement("strong");
    accessoriesTitle.textContent =
      `🎒 Accessoires (${data.accessories.length})`;
    result.appendChild(accessoriesTitle);

    if (!data.accessories.length) {
      const empty = document.createElement("p");
      empty.textContent = "Aucun accessoire.";
    ...
    data.accessories.forEach(item => {
      ...
      text.textContent =
        `${item.item_name} — ${item.active ? "🟢 actif" : "⚪ inactif"} `;
      ...
      remove.onclick = async () => {
        try {
          await api(
            `/api/admin/users/${user.id}/accessory/${encodeURIComponent(item.item_id)}`,
            { method: "DELETE" }
          );

          await searchAdminUser();
        } catch (error) {
          alert(error.message);
        }
      };
    ...
    giveTitle.textContent = "🎁 Donner un accessoire";
    result.appendChild(giveTitle);

    const select = document.createElement("select");
    select.id = "adminAccessorySelect";
    ...
    give.textContent = "🎁 Donner";
    give.onclick = async () => {
      try {
        await api(`/api/admin/users/${user.id}/accessory`, {
          method: "POST",
          body: JSON.stringify({
            itemId: select.value
          })
...
async function sendAdminAnnouncement() {
  const input = document.getElementById("adminAnnouncement");
  const content = input?.value.trim();
  ...
  try {
    ...
    input.value = "";
    alert("📢 Annonce envoyée dans le chat public !");
  } catch (error) {
    alert(error.message);
  }
}
